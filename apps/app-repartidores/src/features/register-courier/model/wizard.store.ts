...
export const useRegisterWizardStore = create<WizardState & WizardActions>((set, get) => ({
  ...
  submit: async () => {
    ...
    try {
      // 1. Armamos el objeto JSON puro (ignorando los File de las imágenes por ahora)
      const payload = {
        email: dto.email,
        passwordRaw: dto.password, // Mapeamos a lo que espera AuthAPI
        rol: 3, // Rol estricto de repartidor
      ...
      // 2. Enviamos el JSON a tu API local
      await submitCourierRegistration(payload);
